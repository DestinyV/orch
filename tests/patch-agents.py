...
patches = [
    ("api-contract-creator.md", "## 职责",
     "## 调用方式\n\n通过 `Agent(subagent_type=\"orch:api-contract-creator\", prompt=\"...\")` 派遣。\n\n```\nAgent(\n  subagent_type=\"orch:api-contract-creator\",\n  prompt=\"将 design.md 中的接口定义转化为接口契约文档，执行六维度审查\"\n)\n```\n\n## 输出\n\n- `api-contract.md` — 接口契约文档\n- `review-report.md` — 六维度审查报告\n\n"),
    ...
    ("code-executor.md", "## 核心流程",
     "## 调用方式\n\n通过 `Agent(subagent_type=\"orch:code-executor\", prompt=\"...\", run_in_background=true)` 派遣。\n\n```\nAgent(\n  subagent_type=\"orch:code-executor\",\n  prompt=\"实现 Task-N，遵循 TDD 流程 (RED-GREEN-REFACTOR-REVIEW)\"\n)\n```\n\n"),
    ("code-explorer.md", "## 职责",
    ...
    ("code-tasker.md", "## 职责",
     "## 调用方式\n\n通过 `Agent(subagent_type=\"orch:code-tasker\", prompt=\"...\")` 派遣。\n\n```\nAgent(\n  subagent_type=\"orch:code-tasker\",\n  prompt=\"将 design.md 拆解为可执行 Task 清单\"\n)\n```\n\n## 约束\n\n<HARD-GATE>每个 Task 必须有 provides/consumes 声明 | 依赖关系必须无环（DAG）</HARD-GATE>\n\n"),
    ("code-tester.md", "## 职责",
     "## 调用方式\n\n通过 `Agent(subagent_type=\"orch:code-tester\", prompt=\"...\", run_in_background=false)` 派遣。\n\n```\nAgent(\n  subagent_type=\"orch:code-tester\",\n  prompt=\"执行高层测试（集成/E2E/性能）并返回测试报告\"\n)\n```\n\n"),
    ("exception-handler.md", "## 职责",
     "## 调用方式\n\n通过 `Agent(subagent_type=\"orch:exception-handler\", prompt=\"...\")` 在 code-execute 内部自动触发。\n\n```\nAgent(\n  subagent_type=\"orch:exception-handler\",\n  prompt=\"扫描 src/ 识别异常场景并生成异常处理代码\"\n)\n```\n\n"),
