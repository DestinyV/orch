...
| Agent | 职责 | 使用时机 |
|-------|------|---------|
| workflow-control | 统一入口 + 流程编排 | 新需求入口 |
| spec-creation | 需求分析和规范生成 | 需求分析阶段 |
| test-designer | 测试设计和规范生成 | 规范完成后 |
| code-architect | 代码设计规划 | 设计阶段 |
| code-tasker | 任务列表生成 | 设计确认后 |
| code-executor | TDD 代码实现 | 任务定义后 |
| code-reviewer | 代码规范+质量审查 | 代码实现后 |
| code-tester | 测试验证和闭环 | 代码审查后 |
| exception-handler | 异常模式扫描+代码生成 | 后端/全栈编码时 |
| knowledge-curator | 知识复利执行 | 工作流完成后 |
| spec-archiver | 规范归档合并 | 测试通过后 |
...
| api-contract-creator | 接口契约定义 | 已有 |
