...
```bash
npx skills add https://github.com/DestinyV/orch.git
```
...
1. **克隆或下载本项目**
```bash
git clone https://github.com/DestinyV/orch.git 
```
...
### 问题1：找不到skill
**症状**：运行 `/spec` 报错 "skill not found"

**解决**：
1. 确认安装完成：`npx skills add https://github.com/DestinyV/orch.git`
2. 重启Claude Code CLI
...
### 问题3：找不到spec目录
**症状**：运行 `/design` 时报错 "spec directory not found"

**解决**：
1. 首先运行 `/spec` 生成spec规范
2. 确保规范文件已生成到 `orch-spec/{requirement_desc_abstract}/spec/` 目录
