...
显示 SDD+TDD 知识复利引擎 v2（knowledge-continuum）已学习的 instincts，包含项目级和全局，按领域分组。
...
### TESTING (3)
  ...
  █████████░  90%  coverage-threshold-85 [project]
            trigger: when code-execute completes
...
- **会话提炼**：`code-execute` 和 `code-test` 执行后自动沉淀经验
- **知识复利**：`knowledge-continuum` 在 `spec-archive` 后自动循环提炼
