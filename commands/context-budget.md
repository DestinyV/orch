---
description: 审计 Claude Code 上下文窗口使用情况：agents/skills/MCP/rules/CLAUDE.md 的 Token 估算。识别膨胀组件，提供优化建议。
---

# Context Budget Command

分析 Claude Code 上下文中每个加载组件的 Token 开销。帮助了解还有多少上下文余量，识别优化空间。
...
### Phase 1: 清单 (Inventory)

扫描所有组件目录，估算 Token 消耗：

**Agents** (`agents/*.md`)
- 统计每文件 lines × 1.3 tokens
...
| 分类 | 标准 | 建议 |
|------|------|------|
| **Always needed** | 被 CLAUDE.md 引用、支撑活跃命令 | 保留 |
| **Sometimes needed** | 领域特定、未被引用 | 考虑按需加载 |
| **Rarely needed** | 无命令引用、内容重叠、无项目匹配 | 移除 / lazy-load |

### Phase 3: 检测问题 (Detect Issues)
