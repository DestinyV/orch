...
- 🔀 [Git-Worktrees指南](./skills/code-execute/references/git-worktrees-guide.md) - 隔离工作环境指南
- ⚡ [Worktrees快速参考](./skills/code-execute/references/quick-reference.md) - 日常使用快速查询卡片
...
**工作流**：
```
/start "需求" → /spec-creation (规范) → /test-design (测试规范) ⟷ /code-design (设计) → /api-contract (契约) → /code-task (任务) → /code-execute (编码) → /exception-handler (异常) → /code-test (测试) → /spec-archive (归档) → 完成
```

> `/test-design` 和 `/code-design` 可并行执行（均依赖 spec-creation 输出）。
> `/exception-handler` 在后端/全栈项目中自动触发（零硬编码，项目约定扫描）。
...
| Skill | 阶段 | 功能 | 输出 |
|-------|------|------|------|
| **script-writer** | 工具 | 工具优先策略：Grep/Bash/Glob/Edit 优先完成文件搜索/提取/编辑/校验，仅脚本无法处理时兜底Read | 脚本化文件操作 |
| **workflow-control** | 编排器 | 统一入口+流程编排：自动检测模式、串联9个Skill、HARD-GATE卡点、中断恢复 | .workflow-state.json + 全流程自动执行 |
...
| **code-design** | Design | 根据规范进行代码设计规划，生成架构和技术方案 | spec-dev/{requirement_desc_abstract}/design/design.md |
| **api-contract** | Api-Contract | 接口契约定义与审查（fullstack强制） | api-contract.md + review-report.md |
...
| **exception-handler** | Exception | 异常场景识别 + 项目约定扫描 + 异常代码生成（后端/全栈，零硬编码） | src/** (添加异常处理) |
...
| Agent | 职责 | 应用场景 |
|-------|------|--------|
| **code-architect** | 通过分析现有代码库的模式和约定，设计功能架构并提供完整的实现蓝图 | 在code-design阶段，分析项目结构、提取设计模式、规划架构 |
| **code-explorer** | 通过追踪执行路径、映射架构层、识别设计模式，深入分析现有代码实现 | 在spec-creation和code-design阶段，提取架构约定和可复用代码 |
| **code-executor** | 根据详细的实现任务，通过TDD流程（RED-GREEN-REFACTOR-REVIEW）编写高质量代码 | 在code-execute阶段，每个Task派遣独立子代理在git-worktree中实现 |
| **code-reviewer** | 针对bug、逻辑错误、安全漏洞和代码质量进行审查，置信度过滤（≥80） | 在code-execute阶段3.3/3.4和code-test阶段2：规范审查+质量审查 |
| **spec-archiver** | 规范归档专家，对标分析、冲突检测、智能合并、一致性验证 | 在spec-archive阶段派遣执行场景对标、合并和一致性检查 |
| **test-designer** | 将TEST-VERIFY转换为测试用例，定义Mock策略，生成fixtures和测试框架代码 | 在test-design阶段分析场景、设计测试用例、生成测试模板 |
| **exception-handler** | 项目约定扫描 + 异常场景识别 + 异常代码生成，零硬编码设计 | 在code-execute阶段后端/全栈Task自动触发，添加异常处理 |
| **api-contract-creator** | 接口契约定义和六维度审查（完整性/命名/类型/错误/约定/数据库） | 在api-contract阶段fullstack强制，生成契约文档和审查报告 |
| **code-tasker** | 设计→任务拆解 + 依赖分析 + 并行执行规划 + Test Case映射 | 在code-task阶段将设计分解为可执行任务和依赖DAG |
...
- 定义接口契约并进行五维度审查
- 输出：`spec-dev/{requirement_desc_abstract}/api-contract/`
...
- 第2步：在code-design阶段分配code-architect进行架构设计
...
- ✅ 7个核心Skills：spec-creation、code-design、test-design、code-task、code-execute、code-test、spec-archive
- ✅ 多阶段审查机制和闭环验证
...
4. 根据提示输入需求内容
5. 逐步执行 spec-creation → code-design → test-design → code-task → code-execute → code-test → spec-archive
6. 选择一个小功能进行完整流程试运行
...
4. 学习 [Git-Worktrees指南](./skills/code-execute/references/git-worktrees-guide.md) 掌握隔离工作环境机制
5. 理解架构设计和工作流内部机制
