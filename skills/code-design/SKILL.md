...
**项目上下文提取**：若 project-context.md 不存在，使用 `Skill("orch:script-writer")` 调用 `extract-project-context.py` 从项目根目录提取技术栈/分层/命名约定。

**补充分析**：基于 spec 文档理解需求，派遣 **code-architect** Agent 进行架构蓝图分析。架构分析中的代码库扫描使用 `Skill("orch:script-writer")` 工具优先策略（Grep搜索→Python3批量过滤→兜底Read）。
