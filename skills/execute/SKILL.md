...
同批次多 Task 派遣时，公共上下文只构造一次，减少编排器重复读取：

1. **批次开始时**：一次性读取批次公共上下文（design-summary / project-map 子图 / exception-patterns / batch-context）
2. **每个 Task 注入时**：注入内容 = 公共上下文（复用） + task-spec（差异部分）
3. **批次全部派遣完成后**：释放公共上下文引用
...
# 每个 Task 仅替换 task_spec 部分
for task in batch:
    injection = batch_ctx + task_spec(task)
