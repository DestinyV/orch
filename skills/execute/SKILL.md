...
3. **TDD 引导加载**（standard 模式）：`Agent(subagent_type="orch:tdd-guide")` 确保四阶段要求注入子代理上下文。
...
- **多批次循环**：Task 超过 3 批次时 → `Skill("orch:ralph-loop")` 接管循环执行
- **失败诊断**：同一 Task 失败 ≥ 2 次 → `Skill("orch:debug")` 执行因果追踪定位根因
- **最大并发数**：≤5 HARD-GATE，避免资源竞争和上下文溢出

<HARD-GATE>standard模式每个Task必须使用子代理(orch:executor)，不允许主上下文直接编码</HARD-GATE>
...
4. **清理孤立**：`git worktree list` 检查并删除遗留 worktree（使用 `bash "${CLAUDE_PLUGIN_ROOT}/skills/execute/scripts/prune-worktrees.sh"`）
