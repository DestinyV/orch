...
**standard模式**：子代理通过 workflow 派遣（每Task独立 code-executor Agent），本文件定义编码策略要求。
...
<HARD-GATE>fullstack时每个Task启动时必须验证 provides/consumes 接口在 contract.md 中存在。失败则拒绝执行。</HARD-GATE>
...
**异常处理要求**（后端/全栈 Task）：
- <HARD-GATE>后端/全栈编码时必须调用 exception Skill 进行异常模式扫描和代码生成</HARD-GATE>
...
- 后端/全栈 Task 必须调用 exception Skill 执行异常模式扫描
