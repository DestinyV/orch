...
**MainAgent TDD验证**（P2.3 anti-forgery）：子代理返回后，MainAgent 必须独立验证：

1. 读取子代理创建的测试文件，确认非空且有真实断言（不是 `// TODO` 骨架）
2. 确认实现文件存在且完整（不是空函数体）
3. **按代码复杂度加权抽样验证**：对满足以下任一条件的函数/文件必须抽查：
   - 函数行数 > 50
   - 分支数（if/switch/case） > 5
   - 涉及外部 API/DB/IO 调用
   - 被 spec 标记为 `@critical` 的路径
   抽样范围为 `max(2, 复杂度权重和评估)` 个函数
4. 子代理 TDD REVIEW 日志必须包含覆盖率工具的 **raw output**（不能只是 `覆盖率92%` 一词概括——必须贴工具输出的逐项覆盖率数据）
5. 任何一项不满足 → 拒绝该 Task，要求子代理重新执行（按增量恢复策略，仅重做不满足的阶段）
...
- 覆盖率达标（行≥85% / 分支≥75% / 函数≥85% / 语句≥85% / @critical 路径 100%）(standard) / ≥60%(quick) | 包含正常+边界+错误
