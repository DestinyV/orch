...
<HARD-GATE>未读取到→暂停确认 | standard模式必须严格执行TDD和子代理，不允许降级</HARD-GATE>
...
<HARD-GATE>未确认测试框架状态→不允许开始编码 | standard模式未编写测试文件→不允许写实现代码</HARD-GATE>

1. **测试框架可用性**：检查项目是否有测试依赖(jest/vitest/pytest等)。无时 AskUserQuestion 确认添加。
2. **测试模板存在性**：<HARD-GATE>standard 模式必须存在 `tests/test-*.template`（由 test-design 阶段生成）。不存在时必须先调用 `Skill("orch:test-design")` 生成，不允许跳过 RED 阶段直接编码。</HARD-GATE>
3. **TDD 引导加载**（standard 模式）：`Skill("orch:tdd-guide")` 确保四阶段要求注入子代理上下文。
4. **工作模式最终确认**：AskUserQuestion 确认执行模式（标准/快速/仅编码）。模式锁定后不可更改。

### 并行执行Task批次

按依赖分批，每批无依赖的Task并行执行。

- **批次内并行**：同一批次所有无依赖Task通过独立子代理同时启动（`run_in_background=true`）
- **并行强制**：<HARD-GATE>standard 模式下同一批次有 N 个无依赖 Task 时必须启动 N 个子代理并行。不允许将并行 Task 串行化到主上下文。</HARD-GATE>
- **批次间串行**：前一批全部完成后才能启动下一批
- **失败策略**：某Task失败阻塞同批次，不继续下一批
- **多批次循环**：Task 超过 3 批次时 → `Skill("orch:continuous-agent-loop")` 接管循环执行
- **失败诊断**：同一 Task 失败 ≥ 2 次 → `Skill("orch:trace")` 执行因果追踪定位根因
- **最大并发数**：≤5 HARD-GATE，避免资源竞争和上下文溢出

<HARD-GATE>standard模式每个Task必须使用子代理(orch:code-executor)，不允许主上下文直接编码</HARD-GATE>

**standard模式**：子代理通过 workflow-control 派遣（每Task独立 code-executor Agent），本文件定义编码策略要求。

**批次派遣流程**：
1. 从 tasks.md 读取所有Task及其依赖
2. 按拓扑排序划分批次（Batch 1 = 无依赖，Batch N = 依赖 Batch N-1）
2.5 **TDD 批次校验**：<HARD-GATE>standard 模式下每批次必须包含对应的测试 Task，不允许所有测试 Task 排在最后批次。若检测到测试 Task 全在末尾 → 重新拆分，将测试 Task 内联到实现 Task 所在批次。</HARD-GATE>
3. 对当前批次所有Task并行派遣子代理（`run_in_background=true`），测试 Task 优先执行（RED 阶段先于 GREEN）
4. 等待全部完成后检查状态
5. 任一失败 → AskUserQuestion 决策（重试/跳过/人工）
6. 全部成功 → 进入下一批

**跨批次 provides/consumes 阻塞**：
- Task 标注 `consumes: API-X` 时，必须等待提供该 API 的 Task（`provides: API-X`）所属批次全部完成
- 阻塞信息在派遣前检查，不满足则推迟该Task到后续批次

**子代理失败重试协议**：
- 第1次失败 → 检查错误原因，补充缺失上下文后重试（最多2次）
- 2次重试均失败 → AskUserQuestion 报告失败原因，等待用户决策（继续/跳过/人工介入）
- ❌ 禁止：主代理绕过子代理直接编码
...
<HARD-GATE>standard 模式必须使用子代理 + worktree 隔离执行。worktree 创建失败时必须执行修复协议，不允许直接降级为主上下文串行编码。</HARD-GATE>

**worktree 创建协议**（顺序尝试，直到成功）：

```bash
# 元凶1: 当前分支已被另一个 worktree 占用 → 需要创建新分支
# 元凶2: .claude/worktrees/ 目录有残留 → 需要清理
...
**关键区别**：尝试 1-2 使用原分支（可能被其他 worktree 占用），尝试 3-4 使用 `-b` 创建新分支 + `git worktree prune` 清理孤立 worktree 记录，确保不与其他 worktree 冲突。

**不允许的降级路径**：
- worktree 失败 → 主上下文编码 → ❌
- worktree 失败 → 串行执行所有 Task → ❌

**允许的降级路径**：
- worktree 失败 → 子代理在同目录工作（run_in_background=true 保持并行）→ ✅
- worktree 失败 → 用 `cp -r` 创建隔离目录替代 → ✅

详见：`references/git-worktrees-guide.md` | `references/worktree-confirmation-protocol.md`

#### 3.1.5 接口契约验证（fullstack强制）

<HARD-GATE>fullstack时每个Task启动时必须验证 provides/consumes 接口在 api-contract.md 中存在。失败则拒绝执行。</HARD-GATE>

验证：读取Task的provides/consumes → 查api-contract.md → 不存在则报错。接口变更必须先更新版本号、重新审查。

#### 3.1.6 SQL验证（数据库Task强制）

<HARD-GATE>数据库Task必须验证 sql-ddl.md：语法正确性 | 回滚脚本存在 | SQL方言一致 | 表设计规范达标。失败则拒绝执行。</HARD-GATE>
...
<HARD-GATE>子代理只能修改Task定义中指定的文件路径，禁止创建或修改清单外的文件</HARD-GATE>
...
| **REVIEW** | Lint + 类型检查 + 覆盖率 ≥ 85% | 0 lint 错误, 0 类型错误, 覆盖率达标, 无 console/print, 无空函数体 | 任一项不满足则阻塞，修复后重检 |
...
**异常处理要求**（后端/全栈 Task）：
- <HARD-GATE>后端/全栈编码时必须调用 exception-handler Skill 进行异常模式扫描和代码生成</HARD-GATE>
- 详见 [`../exception-handler/SKILL.md`](../exception-handler/SKILL.md)

**代码实现要求**：
- TypeScript strict模式 | 遵循项目命名规范 | 无遗留TODO | 无伪代码(console/print)
- **后端语言质量要求**（按技术栈执行对应静态检查）：
  - Python: `pylint` / `ruff` (代码规范) | `mypy` (类型检查) | `black` (格式化)
  - Go: `go vet` (静态分析) | `golangci-lint` (综合检查) | `gofmt` / `goimports` (格式化)
  - Java: `checkstyle` / `spotbugs` | `Error Prone` | Maven/Gradle `compile` 通过
  - Rust: `cargo clippy` | `cargo fmt` | `cargo check` 通过
...
- 编码前执行异常模式扫描，发现项目约定（异常类名、错误码格式、RPC调用模式）
  - 详见 [`../exception-handler/references/exception-pattern-discovery.md`](../exception-handler/references/exception-pattern-discovery.md)
...
**单元测试约束**：
- 覆盖率≥85%（语句/分支/函数） | 正常路径+边界+错误处理 | 每测试一行为
...
- **后端模板**：`../code-test/templates/backend-unit-test-template.md`（Python/Go/Java/Rust 完整示例）
...
#### 3.3 规范审查（派遣 code-reviewer Agent）

<HARD-GATE>standard 模式必须通过 Agent 派遣 code-reviewer，不允许主上下文直接审查。</HARD-GATE>
...
Agent(
  ...
  prompt="
    ...
    2. 关键组件是否按设计实现
    3. 命名、结构是否符合规范
    4. 层间依赖方向是否符合设计（表现层→应用层→领域层）
    
    仅报告置信度 ≥ 80 的问题。返回：问题清单 + 置信度 + 修复建议。
  ",
  run_in_background=false
)
```

#### 3.4 质量审查（派遣 code-reviewer Agent）
...
Agent(
  subagent_type="orch:code-reviewer",
  prompt="
    对 Task-{id} 的代码进行**质量审查**：
    - 代码路径: src/{task-files}
    
    检查维度：
    - 前端：TS strict | ESLint | SOLID/DRY | 无注释-only代码 | 完整性(无空框架)
    - 后端（按技术栈）：
      - Python: pylint ≥ 8.0 | mypy 无错误 | black 格式一致
      - Go: go vet 无告警 | golangci-lint 通过 | gofmt 通过
      - Java: checkstyle 0错误 | spotbugs 0严重问题 | mvn compile 通过
      - Rust: cargo clippy 无警告 | cargo fmt 通过
    - 所有语言：0编译错误 | SOLID/DRY | 无伪代码 | 无注释-only实现
...
<HARD-GATE>standard模式缺失任何阶段→该Task视为未完成</HARD-GATE>

**MainAgent TDD验证**：子代理返回后，MainAgent必须独立验证：
1. 读取子代理创建的测试文件，确认非空且有真实断言（不是 `// TODO` 骨架）
2. 确认实现文件存在且完整（不是空函数体）
3. 随机抽查 1-2 个测试，运行确认能通过
4. 任何一项不满足 → 拒绝该 Task，要求子代理重新执行

### 执行完成前自检

生成报告前必须自检：TDD流程✓ | 测试文件✓ | 测试实际运行✓ | 子代理使用✓ | RED阶段✓ | 覆盖率达标✓ | 审查完成✓ | 修复循环✓ | 分支安全✓ | 无伪代码✓ | TDD日志✓ | 模式标签遵循✓

不通过→暂停回到对应Task重新执行，不允许进入 code-test。
...
4. **清理孤立**：`git worktree list` 检查并删除遗留 worktree（使用 `bash "${CLAUDE_PLUGIN_ROOT}/skills/code-execute/scripts/prune-worktrees.sh"`）
...
- 每Task独立worktree | 严禁跨分支提交 | 严禁worktree中切换分支
- 最大并发数≤5 | 子代理文件范围锁定 | 跨批次provides/consumes阻塞检查
- 严禁console/print伪代码 | 严禁只写注释不实现
- 代码提交前AskUserQuestion确认 | 合并前确认策略
- 两阶段审查(standard)或单阶段(quick) | 修复后重新审查
- 覆盖率≥85%(standard)/≥60%(quick) | 包含正常+边界+错误
- 前端Task必须浏览器测试 | 每Task记录TDD日志 | 完成前必须自检
- standard模式必须子代理 | 必须先验证测试环境 | 禁止跳过RED
- 后端代码必须通过语言对应静态检查：Python(pylint/mypy) | Go(go vet/golangci-lint) | Java(checkstyle/mvn compile) | Rust(cargo clippy)
- 后端/全栈 Task 必须调用 exception-handler Skill 执行异常模式扫描
...
| 参考文档 | 使用场景 | 阶段 |
|---------|---------|------|
| `../exception-handler/SKILL.md` | 异常处理场景和类型判断 | 步骤3.2 |
| `../exception-handler/references/exception-pattern-discovery.md` | 异常模式扫描流程 | 步骤3.2 |
