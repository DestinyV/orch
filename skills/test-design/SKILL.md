...
description: |
  ...
  输出：spec-dev/{requirement_desc_abstract}/tests/test-spec-creation.md + fixtures.json + test-*.template
...
输出 test-spec-creation.md，包含：项目信息 | 测试框架选型 | 测试分层(Unit/Integration/E2E/Browser) | Test Case表格 | Mock/Fixture定义 | 覆盖率分析。
...
**test-spec-creation.md**：所有TEST-VERIFY有对应test case | 测试分层清晰 | BROWSER-TESTABLE覆盖 | 无broken links
...
| `templates/test-spec-template.md` | test-spec-creation.md 输出结构 | 阶段2 |
