...
| **静默注入** | 写入检查清单，不主动 AskUserQuestion | 追加到 spec-creation 内部 checklist |
...
# 或执行脚本
bash "${CLAUDE_PLUGIN_ROOT}/skills/knowledge-continuum/scripts/knowledge-refresh.sh"
```
...
```bash
# 自动触发（每 3 个需求）或手动
bash "${CLAUDE_PLUGIN_ROOT}/skills/knowledge-continuum/scripts/knowledge-distill.sh"
```
