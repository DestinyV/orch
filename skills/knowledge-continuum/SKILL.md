...
test -f "${CLAUDE_PLUGIN_ROOT}/skills/knowledge-continuum/patterns/pattern-index.json" || echo "[WARN] pattern-index.json 缺失"
```
...
python3 -c "
import json, os
idx = json.load(open(os.path.join(os.environ['CLAUDE_PLUGIN_ROOT'], 'skills/knowledge-continuum/patterns/pattern-index.json')))
...
test -f "${CLAUDE_PLUGIN_ROOT}/skills/knowledge-continuum/user-preferences/preferences.json" || echo "[WARN] preferences.json 缺失"
```
...
```json
{
  "hooks": {
    "PreToolUse": [{
      "matcher": "*",
      "hooks": [{ "type": "command", "command": "${CLAUDE_PLUGIN_ROOT}/skills/knowledge-continuum/hooks/observe.sh" }]
    }],
    "PostToolUse": [{
      "matcher": "*",
      "hooks": [{ "type": "command", "command": "${CLAUDE_PLUGIN_ROOT}/skills/knowledge-continuum/hooks/observe.sh" }]
...
```bash
# 查看所有 instincts
python3 "${CLAUDE_PLUGIN_ROOT}/skills/knowledge-continuum/scripts/instinct-cli.py" status

# 集群进化
python3 "${CLAUDE_PLUGIN_ROOT}/skills/knowledge-continuum/scripts/instinct-cli.py" evolve

# 提升到全局
python3 "${CLAUDE_PLUGIN_ROOT}/skills/knowledge-continuum/scripts/instinct-cli.py" promote [id]

# 导出/导入
python3 "${CLAUDE_PLUGIN_ROOT}/skills/knowledge-continuum/scripts/instinct-cli.py" export -o my-instincts.json
python3 "${CLAUDE_PLUGIN_ROOT}/skills/knowledge-continuum/scripts/instinct-cli.py" import my-instincts.json

# 清理过期
python3 "${CLAUDE_PLUGIN_ROOT}/skills/knowledge-continuum/scripts/instinct-cli.py" prune --days 30

# 项目列表
python3 "${CLAUDE_PLUGIN_ROOT}/skills/knowledge-continuum/scripts/instinct-cli.py" projects
```
...
```bash
# 预览
python3 "${CLAUDE_PLUGIN_ROOT}/skills/knowledge-continuum/scripts/instinct-cli.py" promote --dry-run

# 执行
python3 "${CLAUDE_PLUGIN_ROOT}/skills/knowledge-continuum/scripts/instinct-cli.py" promote
```
...
| 文档 | 场景 |
|------|------|
| `references/knowledge-continuum.md` | 完整参考（能力矩阵、设计决策表） |
