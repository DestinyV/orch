...
以下 6 个脚本已内置于 `${CLAUDE_PLUGIN_ROOT}/skills/script-writer/scripts/` 目录，各 Skill 可直接调用，无需重新生成。

| 脚本 | 用途 | 调用 Skill |
|------|------|-----------|
| `${CLAUDE_PLUGIN_ROOT}/skills/script-writer/scripts/extract-test-verify.py` | scenarios → TEST-VERIFY + Mock Data JSON | test-design |
| `${CLAUDE_PLUGIN_ROOT}/skills/script-writer/scripts/validate-outputs.sh` | 批量验证产出文件非空 + 空目录清理 | workflow-control |
| `${CLAUDE_PLUGIN_ROOT}/skills/script-writer/scripts/scan-exceptions.py` | 扫描异常类/错误码/RPC 模式 | exception-handler |
| `${CLAUDE_PLUGIN_ROOT}/skills/script-writer/scripts/extract-api-list.py` | design.md → 接口清单 JSON | api-contract |
| `${CLAUDE_PLUGIN_ROOT}/skills/script-writer/scripts/extract-project-context.py` | 提取技术栈/分层/命名约定 | code-design, workflow-control |
| `${CLAUDE_PLUGIN_ROOT}/skills/script-writer/scripts/extract-eval-metrics.py` | .workflow-eval.json → 指标摘要 | workflow-control |

**调用方式**：
```bash
python3 "${CLAUDE_PLUGIN_ROOT}/skills/script-writer/scripts/extract-test-verify.py" spec-dev/{req}/spec/scenarios/
bash "${CLAUDE_PLUGIN_ROOT}/skills/script-writer/scripts/validate-outputs.sh" file1 file2 --dir dir1 dir2
python3 "${CLAUDE_PLUGIN_ROOT}/skills/script-writer/scripts/scan-exceptions.py" src/
```
...
```bash
Skill("orch:script-writer", args="cross-check:检查 references/ 下所有 .md 文件是否在 SKILL.md 中被引用")
Skill("orch:script-writer", args="batch-extract:提取所有 spec-*/spec/requirement.md 中的 project-mode 字段")
Skill("orch:script-writer", args="validate:批量校验 spec-*/spec/ 下所有 .json 文件")
```
...
<HARD-GATE>禁止 Bash(grep/rg) 替代 Grep 工具 | 禁止复杂 Bash 循环替代 Python3 脚本</HARD-GATE>