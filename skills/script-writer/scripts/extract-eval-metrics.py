...
def diagnose(stages, events, summary):
    ...
    # 并行调度诊断
    parallel = [s for s in stages if s.get('stage') in ('test-design', 'code-design') and s.get('status') == 'done']
    if len(parallel) == 2:
        times = []
        for s in parallel:
        ...
        if len(times) == 2 and abs((times[0] - times[1]).total_seconds()) > 10:
            d['warnings'].append('test-design/code-design 疑似串行执行，未启用并行调度')

    # 覆盖率诊断
    exec_stage = next((s for s in stages if s.get('stage') == 'code-execute'), {})
