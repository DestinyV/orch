...
```bash
Agent(subagent_type="orch:socratic-clarifier",
      prompt="
        对第{n}轮访谈结果进行模糊度评分：
        - 原始需求: {initial_idea}
        - 本轮回答: {answer}
...
```bash
Skill("orch:spec-creation", args="{requirement_desc} (clarification: spec-dev/{req_id}/spec/clarification.md)")
```
...
| 完成后桥接 spec-creation | 跳过 spec-creation 直接编码 |
