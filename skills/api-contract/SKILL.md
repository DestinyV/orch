...
**工具优先**：使用 `Skill("orch:script-writer")` 调用 `extract-api-list.py` 从 design.md 提取接口清单 JSON，替代 AI 逐文件 Read。
...
Agent(
  ...
  prompt="
    ...
    模板见 templates/api-contract-template.md 和 templates/review-report-template.md
  ",
...
模板见 `templates/diagrams/`，输出到 `spec-dev/{req_id}/api-contract/diagrams/`。触发规则见 `../code-design/references/diagram-trigger-rules.md`。
...
| 参考文档 | 使用场景 | 阶段 |
|---------|---------|------|
| `templates/api-contract-template.md` | api-contract.md 输出结构（路径/方法/参数/响应/错误码） | Phase 2 |
