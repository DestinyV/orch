...
6. 检测需求模糊度 → 模糊 > 0.2 时派遣 `socratic-clarify`；否则级联 `spec-creation`
...
```bash
Skill("orch:socratic-clarify", args="{requirement_desc}")
```

产出校验：`clarification.md` 存在且含 `final_ambiguity`。完成后自动级联 `spec-creation`。
...
| 步骤 | 前置 | 产出 | 记录 | 纠正 |
|------|------|------|------|------|
| 1 spec-creation | state.json 已初始化 | requirement/scenarios/data/business/glossary 非空 + 模式标签 + 异常场景 | stage/stats/agent 写入 eval.json | 缺失回溯 |
| 2-3 parallel | scenarios 含 TEST-VERIFY | test-spec非空+fixtures可解析 / design.md含架构+组件+决策 | stage/stats/agent×2 写入 eval.json | 缺失重新派遣 |
| 3.5 api-contract | code-design done + fullstack | api-contract.md + review-report无blocking | stage/stats 写入 eval.json | blocking修复后重审 |
| 4 code-task | design.md 存在 | tasks.md 每Task有 provides/consumes/验收标准/DAG | stage/stats 写入 eval.json | 补全后继续 |
| 5 code-execute | tasks.md存在 + TDD出口验证 | src/非空 + execution-report.md + TDD四阶段日志(RED/GREEN/REFACTOR/REVIEW) | stage/batch_stats 写入 eval.json | 回溯/补测试/拆批次 |
| 5.5 exception | 后端/全栈自动 | 异常代码生成 | — | — |
| 6 code-test | src/存在 + report存在 | testing-report.md存在 + E2E执行 | stage/stats/agent×2 写入 eval.json | 失败回code-execute |
| 7 spec-archive | 全部测试通过 | 主规范已合并 + archive-log.md | stage/stats 写入 eval.json | 失败回溯 |
| 8 evaluation | spec-archive done + eval.json 含全阶段数据 | diagnosis字段已写入 + context-budget + cost-tracking | 汇总诊断报告 | stages[]为空则回溯 |
...
| 3.5 | `api-contract-creator` | `run_in_background=false` |
| 4 | `code-tasker` | `run_in_background=false` |
| 5 | `code-executor` ×N + `code-reviewer` | `run_in_background=true` ×N |
| 5.5 | `exception-handler` | 子过程自动 |
| 6 | `code-tester` + `test-verifier` | `run_in_background=false` |
| 7 | `spec-archiver` | `run_in_background=false` |
| 9 | `knowledge-curator` | `run_in_background=false` |

| 辅助 Agent | 触发条件 | 集成点 |
|-----------|---------|--------|
| `socratic-clarifier` | 模糊度 > 0.2 | socratic-clarify |
| `tracer` | Task 失败 ≥ 2 次 | code-execute |
| `e2e-runner` | 前端/全栈 E2E | code-test |
| `loop-operator` | Task > 3 批次 | code-execute |
| `planner` | 用户要求 | code-design |
| `tdd-guide` | standard 模式, 每批次审查TDD四阶段日志, 不满足驳回 | code-execute 每批次 | | code-execute 启动 |
...
for task in $(python3 -c "import json;tasks=json.load(open('spec-dev/{req}/tasks/tasks.json'));[print(t['id']) for t in tasks if not t.get('depends_on')]"); do
  Agent(subagent_type="orch:code-executor", run_in_background=true,
...
# ═══ 步骤7: spec-archive ═══
Agent(subagent_type="orch:spec-archiver",
...
6. 上下文过大时，中断恢复前先调用 `Skill("orch:context-budget")` 审计 → `Skill("orch:strategic-compact")` 建议 compaction 后再继续

## 快速模式

跳过 test-design | 跳过 api-contract | code-execute 单审查/覆盖≥60% | code-test 精简（集成+E2E）。
