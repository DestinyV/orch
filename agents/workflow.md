...
<HARD-GATE>零预探索契约：被调用时禁止在此之前已执行任何文件读取、目录扫描或项目分析。需求来自 args 参数，项目探索由 spec 阶段(code-explorer)负责。</HARD-GATE>
...
<HARD-GATE>禁止跳过阶段 | 禁止在正式流程前执行代码探索 | 禁止在调用前已读取过任何项目文件</HARD-GATE>
