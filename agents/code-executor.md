...
阅读 tasks.md 中当前 Task 的目标/交付物/验收标准 | 理解 code-design 的设计规范和接口定义 | 参考 code-architect 的架构蓝图 | 理解集成点 | 审查项目约定（CLAUDE.md）。
...
**异常处理**（后端/全栈 Task）：
- 编码前必须调用 `Skill("orch:exception-handler")` 执行异常模式扫描
