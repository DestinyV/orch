...
通过 `Agent(subagent_type="orch:exception-handler", prompt="扫描 src/ 识别异常场景并生成异常处理代码")` 在 code-execute 内部自动触发。
...
编码前先扫描项目，发现异常处理约定。**工具优先**：搜索用 Grep，批量过滤用 Python3 脚本，避免逐个文件 Read。详见 `Skill("orch:script-writer")`。
...
详见：[`../skills/exception-handler/references/exception-pattern-discovery.md`](../skills/exception-handler/references/exception-pattern-discovery.md)
